...
alter table public.offer_rules add column if not exists threshold_quantity numeric(14, 4) default 0;
...
update public.offer_rules set threshold_quantity = 0 where threshold_quantity is null;
update public.offer_rules set threshold_quantity = 0 where threshold_quantity < 0;
update public.offer_rules set threshold_type = 'EXACT' where threshold_type is null;
alter table public.offer_rules alter column threshold_quantity set default 0;
...
create table if not exists public.promotion_offer_sku_settings (
  ...
  sku text not null,
  segment text not null default ' - ',
  threshold_quantity numeric(14, 4) not null default 0,
...
alter table public.promotion_offer_sku_settings add column if not exists threshold_quantity numeric(14, 4) default 0;
...
update public.promotion_offer_sku_settings set threshold_quantity = 0 where threshold_quantity is null;
update public.promotion_offer_sku_settings set threshold_quantity = 0 where threshold_quantity < 0;
